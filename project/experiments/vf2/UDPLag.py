from networkx.algorithms import isomorphism
import networkx as nx
import numpy as nump
import csv
import matplotlib.pyplot as plot


def udp_lag_vf2(begin, end):
    with open('../../../data/CSV-01-12/01-12/UDPLag.csv', newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=' ', quotechar='|')
        c = 0   # c counts for each edge
        syn_graph = nx.MultiDiGraph()
        for row in reader:
            elements_1 = row[0].split(",")
            if 0 <= c:
                if begin <= c < end:
                    client_ip = elements_1[2]
                    server_ip = elements_1[4]
                    client_port = elements_1[3]
                    server_port = elements_1[5]
                    syn_graph.add_node("a" + client_ip)
                    syn_graph.add_node("s" + server_ip)
                    syn_graph.add_edge(client_ip + client_port, server_ip + server_port)
            else:
                break
            c = c + 1

    # nx.draw(syn_graph, node_color='y', edge_color='b')
    # plot.show()
    return syn_graph


# g1 = udp_lag_vf2(begin=1000, end=1010)
# g2 = udp_lag_vf2(begin=2000, end=2010)
# M = isomorphism.MultiDiGraphMatcher(g1, g2)
# print(M.is_isomorphic())
